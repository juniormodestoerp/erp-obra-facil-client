...
import { COST_AND_PROFIT_CENTERS_QUERY_KEY, type CostAndProfitCentersQueryData } from '@app/hooks/cost-and-profit-centers/use-cost-and-profit-centers'
...
export function useCreateCostAndProfitCenter() {
	...
	const { mutateAsync, isPending } = useMutation({
		...
		onSuccess: async (data, _variables, context) => {
			await queryClient.cancelQueries({ queryKey: COST_AND_PROFIT_CENTERS_QUERY_KEY })

			queryClient.setQueryData<CostAndProfitCentersQueryData>(
				COST_AND_PROFIT_CENTERS_QUERY_KEY,
				(old) =>
					old?.map((costAndProfitCenter) =>
						costAndProfitCenter.id === context.tmpCostAndProfitCenterId ? data : costAndProfitCenter,
					),
			)
		},

...
import { EditCategoryDialog } from './components/edit-bank-account-dialog'
import { RemoveCategoryDialog } from './components/remove-bank-account-dialog'
...
import { useBankAccountsController } from './use-bank-account-controller'
...
export function BankAccounts() {
	...
	const { currentTab, setCurrentTab, categories } = useBankAccountsController()
	...
	function organizeIncomeBankAccounts(categories: ICategory[]) {
		if (!categories) {
			return []
		}
	...
	const organizedIncomeBankAccounts = organizeIncomeBankAccounts(
		categories ?? [],
	)

	function organizeExpenseBankAccounts(categories: ICategory[]) {
	...
	const organizedExpenseBankAccounts = organizeExpenseBankAccounts(
		categories ?? [],
	)
	...
	return (
		<Fragment>
			...
			<div className="my-8 h-auto border-collapse overflow-hidden rounded border shadow dark:border-slate-400 dark:bg-slate-800">
				{currentTab === 'Receitas'
					? organizedIncomeBankAccounts.length > 0 &&
						organizedIncomeBankAccounts.map((category: ICategory) => {
					...
					: organizedExpenseBankAccounts.length > 0 &&
						organizedExpenseBankAccounts.map((category: ICategory) => {
							const isPrimary = !category.categoryId
							return (
								<div
									key={category.id}
